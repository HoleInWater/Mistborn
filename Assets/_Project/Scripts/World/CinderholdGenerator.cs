...
/// <summary>
/// Procedural city generator for Cinderhold — the capital of the Ashen Dominion.
/// Creates a grid-based city with noble keeps, lowborn slums, canals, and ashfall.
/// Buildings have metal fixtures (pushable/pullable) for Metallurgic traversal.
///
/// Lore: Cinderhold is divided into districts — noble keeps in the center,
/// lowborn slums on the outskirts, canals cutting through, and Thornspire
/// (the Ashen King's palace) at the heart.
/// </summary>
public class CinderholdGenerator : MonoBehaviour
{
    [Header("City Layout")]
    ...
    public GameObject[] lowbornHousePrefabs;
    ...
    [Header("Landmarks")]
    public GameObject thornspirePrefab;
    ...
    public Color lowbornDistrictColor = new Color(0.3f, 0.3f, 0.3f);
    ...
    public enum DistrictType { Noble, Lowborn, Canal, Market, Keep, Road, Fortress }
    ...
    public void GenerateCity()
    {
        ...
        cityParent = new GameObject("Cinderhold").transform;
        ...
        // Phase 4: Place metal fixtures (for Metallurgic traversal)
        PlaceMetalFixtures();
    ...
    void AssignDistricts()
    {
        ...
        for (int x = 0; x < cityWidth; x++)
        {
            for (int z = 0; z < cityDepth; z++)
            {
                ...
                {
                    cityGrid[x, z].type = DistrictType.Lowborn;
            ...
            for (int z = 0; z < cityDepth; z++)
            {
                ...
                switch (block.type)
                {
                    case DistrictType.Noble:
                        ...
                        break;
                    case DistrictType.Lowborn:
                        PlaceBuilding(pos, lowbornHousePrefabs, block.buildingHeight, block);
                        break;
    ...
    void PlaceBuilding(Vector3 pos, GameObject[] prefabs, float height, CityBlock block)
    {
        if (prefabs == null || prefabs.Length == 0)
        {
            ...
            if (r != null)
            {
                ...
                mat.color = block.type == DistrictType.Noble ? nobleDistrictColor : lowbornDistrictColor;
                r.material = mat;
            }

            // Add collider for Metallurgic traversal
    ...
    void PlaceMetalFixtures()
    {
        for (int x = 0; x < cityWidth; x++)
        {
            for (int z = 0; z < cityDepth; z++)
            {
                ...
                MetallurgicTarget target = fixture.GetComponent<MetallurgicTarget>();
                if (target == null) target = fixture.AddComponent<MetallurgicTarget>();
        ...
        for (int i = 0; i < coinCount; i++)
        {
            ...
            coinRb.mass = MetallurgyPhysicsFormulas.COIN_MASS;

            MetallurgicTarget ct = coin.AddComponent<MetallurgicTarget>();
            ct.canBePushed = true;
            ct.canBePulled = true;
    ...
    void PlaceLandmarks()
    {
        // Thornspire at center
        Vector3 center = new Vector3(cityWidth * blockSize * 0.5f, 0, cityDepth * blockSize * 0.5f);

        if (thornspirePrefab != null)
        {
            GameObject ks = Instantiate(thornspirePrefab, center, Quaternion.identity, cityParent);
        ...
        else
        {
            ...
            for (int i = 0; i < 12; i++)
            {
                ...
                spire.name = $"Thornspire_Spire_{i}";
                ...
                rb.isKinematic = true;
                rb.mass = 10000f;
                MetallurgicTarget at = spire.AddComponent<MetallurgicTarget>();
                at.isAnchored = true;
