...
/// <summary>
/// Branching dialogue manager with flag-gated responses, quest triggers, and skill checks.
/// </summary>
public class BranchingDialogueManager : MonoBehaviour
{
    ...
    bool IsResponseAvailable(DialogueResponse response)
    {
        if (!string.IsNullOrEmpty(response.requiredFlag))
            return dialogueFlags.ContainsKey(response.requiredFlag) && dialogueFlags[response.requiredFlag];
        return true;
    }
    ...
    public bool HasFlag(string flagName) => dialogueFlags.ContainsKey(flagName) && dialogueFlags[flagName];
    public void SetFlag(string flagName, bool value) => dialogueFlags[flagName] = value;

    public void LoadDialogue(Dialogue dialogue)
    {
        if (dialogue != null) loadedDialogues[dialogue.dialogueId] = dialogue;
    }
}
