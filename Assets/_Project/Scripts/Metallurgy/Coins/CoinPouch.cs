...
/// <summary>
/// Launcher's primary weapon system. Carries coins that can be individually Pushed.
/// Supports: single throw, shotgun spread, ground bounce, and rapid-fire trail.
/// </summary>
[PlayerComponent("Metallurgy Support", order: 50)]
public class CoinPouch : MonoBehaviour
{
    ...
    void ThrowAndPushCoin()
    {
        ...
        if (coinRb != null && playerRigidbody != null)
        {
            ...
            float coinVel = MetallurgyPhysicsFormulas.CalculateCoinVelocity(force, 2f, coinMass);
            coinRb.AddForce(dir * coinVel * coinMass, ForceMode.Impulse);

            // Newton's 3rd Law mass ratios
            float playerRatio, objectRatio;
            MetallurgyPhysicsFormulas.CalculateMassRatios(
    ...
    GameObject SpawnCoin(Vector3 position)
    {
        ...
        if (coin.GetComponent<MetallurgicTarget>() == null)
        {
            MetallurgicTarget target = coin.AddComponent<MetallurgicTarget>();
    ...
    void DrainMetal(float amount)
    {
        if (metallurgist != null)
            metallurgist.DrainMetal(MetallurgySkill.MetalType.Steel, amount);
    }
