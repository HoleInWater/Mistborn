...
/// </summary>
[PlayerComponent("Metallurgy", order: 40)]
public class MetalReserve : MonoBehaviour
{
    ...
    [Header("Preview Mode")]
    [Tooltip("Auto-enabled when no Metallurgist is found. Animates the ring without full game setup.")]
    ...
    private MetallurgySkill.MetalType _primaryMetal   = MetallurgySkill.MetalType.Steel;
    private MetallurgySkill.MetalType _secondaryMetal = MetallurgySkill.MetalType.Iron;
    private MetallurgySkill.MetalType _activeMetal    = MetallurgySkill.MetalType.Steel;
    ...
    private Dictionary<MetallurgySkill.MetalType, ProgressBar> _metalBars =
        new Dictionary<MetallurgySkill.MetalType, ProgressBar>();
    ...
    private static readonly Color[] MetalColors =
    {
        ...
        new Color(0.86f, 0.86f, 0.94f), // Oraculum
        new Color(0.67f, 0.31f, 0.16f), // Revelum
    ...
    static Color MetalColor(MetallurgySkill.MetalType m)
    {
        int i = (int)m;
    ...
    void Start()
    {
        ...
        if (!previewMode
            && GetComponent<Metallurgist>() == null
            && GetComponentInParent<Metallurgist>() == null)
        {
            previewMode = true;
        }
        ...
        if (previewMode)
        {
            int count = System.Enum.GetValues(typeof(MetallurgySkill.MetalType)).Length;
    ...
    public void HighlightSelection(MetallurgySkill.MetalType primary,
                                   MetallurgySkill.MetalType secondary,
    ...
    public void VisualizePrimedState(MetallurgySkill.MetalType metal, bool isPrimed)
    {
    ...
    private float GetReservePct(MetallurgySkill.MetalType metal)
    {
    ...
    private void UpdateSingleBar(ProgressBar bar, MetallurgySkill.MetalType metal, float[] reserves)
    {
        if (bar == null) return;
        int idx = (int)metal;
    ...
    private void ApplyBarFillColor(ProgressBar bar, MetallurgySkill.MetalType metal)
    {
        if (bar == null) return;
    ...
    {
        var allo = GetComponent<Metallurgist>();
