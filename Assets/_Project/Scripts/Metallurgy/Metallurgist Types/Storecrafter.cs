 ...
 *
 * PURPOSE:
 * Core Storecraft system that manages metalminds, storing/tapping attributes,
 * and integration with Metallurgist for Compounding detection.
 *
 * LORE-ACCURATE FERUCHEMY:
 * ========================
 * Storecraft is end-neutral — it doesn't create or destroy power, only stores
 * and retrieves it. A Storecrafter stores an attribute (speed, weight, health, etc.)
 ...
 * - A character with BOTH Metallurgy and Storecraft for the same metal can Compound
 *
 * METALS & ATTRIBUTES:
...
/// <summary>
/// The 16 Storecrafted attributes, one per metal.
/// Enum order matches MetallurgySkill.MetalType for easy cross-referencing.
/// </summary>
public enum StorecraftedAttribute
{
    Speed,          // Steel   (0)
    Weight,         // Iron    (1)
    Strength,       // Pewter  (2)
    ...
    Health,         // Gold    (8) — Ashen King's immortality via compounding
    Determination,  // Electrum(9)
    Fortune,        // Chromium(10) — mapped to Oraculum's slot for gameplay alignment
    Investiture,    // Nicrosil(11) — mapped to Maloraculum's slot for gameplay alignment
    // The following use indices 12-15 to align with MetallurgySkill.MetalType 12-17
...
/// <summary>
/// Represents a single metalmind that stores one Storecrafted attribute.
/// </summary>
[Serializable]
public class Metalmind
{
    public StorecraftedAttribute attribute;
    ...
    public Metalmind(StorecraftedAttribute attr, float capacity, float storeRate, float tapRate, float diminishing)
    {
        attribute = attr;
...
/// <summary>
/// Core Storecraft component. Attach to any character that can use Storecraft.
/// Manages 16 metalminds with store/tap mechanics and diminishing returns.
/// </summary>
public class Storecrafter : MonoBehaviour
{
    // ═══════════════════════════════════════════════════════════════════════════
    // CONSTANTS
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    // Cached reference to Metallurgist for Compounding detection
    ...
    void Awake()
    {
        AshwalkerRegistry.RegisterStorecrafter(this);
    ...
    void OnDestroy()
    {
        AshwalkerRegistry.UnregisterStorecrafter(this);
    }
    ...
    private void InitializeMetalminds()
    {
        ...
        metalminds[0]  = new Metalmind(StorecraftedAttribute.Speed,         600f, 12f, 18f, 0.04f);  // Steel
        metalminds[1]  = new Metalmind(StorecraftedAttribute.Weight,        800f, 15f, 20f, 0.03f);  // Iron
        metalminds[2]  = new Metalmind(StorecraftedAttribute.Strength,      600f, 12f, 18f, 0.04f);  // Pewter
        metalminds[3]  = new Metalmind(StorecraftedAttribute.Senses,        400f, 10f, 15f, 0.05f);  // Tin
        metalminds[4]  = new Metalmind(StorecraftedAttribute.MentalSpeed,   300f,  8f, 12f, 0.06f);  // Zinc
        metalminds[5]  = new Metalmind(StorecraftedAttribute.Warmth,        400f, 10f, 15f, 0.05f);  // Brass
        metalminds[6]  = new Metalmind(StorecraftedAttribute.Memories,      500f, 10f, 15f, 0.05f);  // Copper
        metalminds[7]  = new Metalmind(StorecraftedAttribute.Wakefulness,   400f, 10f, 15f, 0.05f);  // Bronze
        metalminds[8]  = new Metalmind(StorecraftedAttribute.Health,        700f, 10f, 20f, 0.03f);  // Gold — key for Ashen King
        metalminds[9]  = new Metalmind(StorecraftedAttribute.Determination, 300f,  8f, 12f, 0.06f);  // Electrum
        metalminds[10] = new Metalmind(StorecraftedAttribute.Fortune,       200f,  5f, 10f, 0.08f);  // Chromium
        metalminds[11] = new Metalmind(StorecraftedAttribute.Investiture,   200f,  5f, 10f, 0.08f);  // Nicrosil
        metalminds[12] = new Metalmind(StorecraftedAttribute.Identity,      250f,  6f, 10f, 0.07f);  // Aluminum
        metalminds[13] = new Metalmind(StorecraftedAttribute.Connection,    250f,  6f, 10f, 0.07f);  // Duralumin
        metalminds[14] = new Metalmind(StorecraftedAttribute.Breath,        400f, 10f, 15f, 0.05f);  // Bendalloy
        metalminds[15] = new Metalmind(StorecraftedAttribute.Energy,        500f, 12f, 18f, 0.04f);  // Cadmium
    }
    ...
    private void ProcessStoringAndTapping()
    {
        ...
        for (int i = 0; i < MetalmindCount; i++)
        {
            ...
            if (isStoring[i] && mind.CanStore)
            {
                ...
                // Reduce the attribute while storing (Storecraft is end-neutral)
    ...
    /// </summary>
    public float GetAttributeModifier(StorecraftedAttribute attr)
    {
    ...
    /// Requires both Metallurgy and Storecraft for the same metal.
    /// </summary>
    public bool CanCompound(int metalIndex)
    {
        if (metallurgist == null) metallurgist = GetComponent<Metallurgist>();
        if (metallurgist == null) return false;
        if (metalIndex < 0 || metalIndex >= MetalmindCount) return false;
        if (!unlockedMetals[metalIndex]) return false;

        // Must have Metallurgic access to this metal
        MetallurgySkill.MetalType metalType = MetalIndexToMetallurgicType(metalIndex);
        return metallurgist.unlockedMetals[(int)metalType];
    }

    /// <summary>
    /// Check if this character is actively Compounding a specific metal.
    /// Active Compounding = burning the metal Metallurgically while having a charged metalmind.
    /// </summary>
    public bool IsCompounding(int metalIndex)
    {
        ...
        MetallurgySkill.MetalType metalType = MetalIndexToMetallurgicType(metalIndex);
        return metallurgist.IsMetalBurning(metalType);
    }
    ...
    /// <summary>
    /// Unlock a Storecrafted metal for this character.
    /// </summary>
    ...
    /// <summary>
    /// Maps Storecrafter metal index (0-15) to the corresponding MetallurgySkill.MetalType.
    /// Indices 0-7 map directly. Indices 8-9 map to Gold/Electrum (skipping Oraculum/Maloraculum
    /// which are god metals without Storecrafted equivalents). Indices 10-15 map to the
    /// remaining standard metals.
    /// </summary>
    public static MetallurgySkill.MetalType MetalIndexToMetallurgicType(int metalIndex)
    {
        switch (metalIndex)
        {
            case 0:  return MetallurgySkill.MetalType.Steel;
            case 1:  return MetallurgySkill.MetalType.Iron;
            case 2:  return MetallurgySkill.MetalType.Pewter;
            case 3:  return MetallurgySkill.MetalType.Tin;
            case 4:  return MetallurgySkill.MetalType.Zinc;
            case 5:  return MetallurgySkill.MetalType.Brass;
            case 6:  return MetallurgySkill.MetalType.Copper;
            case 7:  return MetallurgySkill.MetalType.Bronze;
            case 8:  return MetallurgySkill.MetalType.Gold;
            case 9:  return MetallurgySkill.MetalType.Electrum;
            case 10: return MetallurgySkill.MetalType.Chromium;
            case 11: return MetallurgySkill.MetalType.Nicrosil;
            case 12: return MetallurgySkill.MetalType.Aluminum;
            case 13: return MetallurgySkill.MetalType.Duralumin;
            case 14: return MetallurgySkill.MetalType.Bendalloy;
            case 15: return MetallurgySkill.MetalType.Cadmium;
            default: return MetallurgySkill.MetalType.Steel;
        }
    }

    /// <summary>
    /// Maps MetallurgySkill.MetalType to Storecrafter metal index.
    /// Returns -1 for god metals (Oraculum, Maloraculum) which have no Storecrafted equivalent.
    /// </summary>
    public static int MetallurgicTypeToMetalIndex(MetallurgySkill.MetalType metalType)
    {
        switch (metalType)
        {
            case MetallurgySkill.MetalType.Steel:     return 0;
            case MetallurgySkill.MetalType.Iron:      return 1;
            case MetallurgySkill.MetalType.Pewter:     return 2;
            case MetallurgySkill.MetalType.Tin:        return 3;
            case MetallurgySkill.MetalType.Zinc:       return 4;
            case MetallurgySkill.MetalType.Brass:      return 5;
            case MetallurgySkill.MetalType.Copper:     return 6;
            case MetallurgySkill.MetalType.Bronze:     return 7;
            case MetallurgySkill.MetalType.Gold:       return 8;
            case MetallurgySkill.MetalType.Electrum:   return 9;
            case MetallurgySkill.MetalType.Chromium:   return 10;
            case MetallurgySkill.MetalType.Nicrosil:   return 11;
            case MetallurgySkill.MetalType.Aluminum:   return 12;
            case MetallurgySkill.MetalType.Duralumin:  return 13;
            case MetallurgySkill.MetalType.Bendalloy:  return 14;
            case MetallurgySkill.MetalType.Cadmium:    return 15;
            case MetallurgySkill.MetalType.Oraculum:      return -1; // God metal
            case MetallurgySkill.MetalType.Maloraculum:   return -1; // God metal
            default: return -1;
        }
    }
    ...
    /// </summary>
    public static string GetAttributeDescription(StorecraftedAttribute attr)
    {
        switch (attr)
        {
            case StorecraftedAttribute.Speed:         return "Physical speed — storing makes you sluggish, tapping makes you a blur";
            case StorecraftedAttribute.Weight:        return "Body weight — storing makes you lighter, tapping makes you heavier";
            case StorecraftedAttribute.Strength:      return "Physical strength — storing weakens you, tapping grants immense power";
            case StorecraftedAttribute.Senses:        return "Sensory acuity — storing dulls senses, tapping sharpens them beyond normal";
            case StorecraftedAttribute.MentalSpeed:   return "Cognitive speed — storing makes you slow-witted, tapping accelerates thought";
            case StorecraftedAttribute.Warmth:        return "Body heat — storing makes you cold, tapping warms you";
            case StorecraftedAttribute.Memories:      return "Memories — storing removes memories temporarily, tapping recalls with perfect clarity";
            case StorecraftedAttribute.Wakefulness:   return "Wakefulness — storing makes you drowsy, tapping keeps you alert for days";
            case StorecraftedAttribute.Health:        return "Health and vitality — storing makes you sickly, tapping heals wounds rapidly";
            case StorecraftedAttribute.Determination: return "Determination — storing makes you apathetic, tapping fills you with resolve";
            case StorecraftedAttribute.Fortune:       return "Fortune — storing brings bad luck, tapping brings good luck";
            case StorecraftedAttribute.Investiture:   return "Investiture — storing reduces magical power, tapping amplifies it";
            case StorecraftedAttribute.Identity:      return "Identity — storing blurs your sense of self, tapping solidifies it";
            case StorecraftedAttribute.Connection:    return "Connection — storing isolates you, tapping lets you bond with others instantly";
            case StorecraftedAttribute.Breath:        return "Breath — storing makes breathing labored, tapping lets you hold breath indefinitely";
            case StorecraftedAttribute.Energy:        return "Energy — storing makes you fatigued, tapping gives boundless stamina";
