...
/// <summary>
/// Metallurgic flight — the core Ashwalker traversal fantasy.
...
/// so the Metallurgist receives all the force.
///
/// Lore: Ember describes it as "leaping between invisible anchors in the sky."
/// </summary>
public class MetallurgicFlight : MonoBehaviour
{
    [Header("Push Launch")]
    ...
    void Update()
    {
        ...
        bool hasSteelReserve = metallurgist != null && metallurgist.GetMetalReserve(MetallurgySkill.MetalType.Steel) > 0;
        bool hasIronReserve = metallurgist != null && metallurgist.GetMetalReserve(MetallurgySkill.MetalType.Iron) > 0;
    ...
    void TryPushLaunch()
    {
        ...
        if (bestAnchor != null)
        {
            ...
            float force = MetallurgyPhysicsFormulas.CalculateMetallurgicForce(
                MetallurgyPhysicsFormulas.A_CONSERVATIVE * flare,
    ...
    void DrainFlightMetal(bool pushing, bool pulling)
    {
        ...
        if (pushing) metallurgist.DrainMetal(MetallurgySkill.MetalType.Steel, drain);
        if (pulling) metallurgist.DrainMetal(MetallurgySkill.MetalType.Iron, drain);
    }
