...
/// <summary>
/// Implements the Oraculum Metallurgy ability (Seer).
/// Shows "Oraculum Shadows" (future ghosts) of nearby entities.
/// Standardized to follow the Metallurgist-centric burn system.
/// </summary>
[PlayerComponent("Metallurgy Metals", order: 90)]
public class Oraculum : MonoBehaviour
{
    [Header("Settings")]
    public float baseVisionRange = 25f;
    public float ghostAlpha = 0.3f;
    public float maxVisionRange = 50f;
    public float targetTimeScale = MetallurgyConstants.OraculumTimeScale;
    public float shadowLeadTime = MetallurgyConstants.OraculumShadowLeadTime;
    
    [Header("Oraculum Dilation")]
    ...
    void Update()
    {
        ...
        isBurning = metallurgist != null && metallurgist.IsBurning() && metallurgist.GetCurrentMetal() == MetallurgySkill.MetalType.Oraculum;
    ...
    private void HandleTimeDilation(bool active)
    {
        ...
        if (AshwalkerTimeManager.Instance != null)
            AshwalkerTimeManager.Instance.SetOraculumModifier(target);
        else
