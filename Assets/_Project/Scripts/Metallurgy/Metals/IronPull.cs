 ...
 *
 * PURPOSE:
 * Implements the Iron Metallurgy ability (Hauler) – pull metal objects toward the player.
 *
 * TARGETING LINE:
...
[PlayerComponent("Metallurgy Metals", order: 20)]
public class IronPull : MonoBehaviour
{
    ...
    public float metalCostPerSecond = MetallurgyConstants.IronDrainRate; // MAG: 20 min burn (~0.0833/s)
    ...
    private KeyCode GetAbility1Key()
    {
        ...
        {
            if (metalSelector.GetPrimaryMetal()   == MetallurgySkill.MetalType.Iron) return Keybinds.Ability1;
            if (metalSelector.GetSecondaryMetal() == MetallurgySkill.MetalType.Iron) return Keybinds.Ability2;
            return KeyCode.None;
        }
    ...
    void Update()
    {
        ...
        if (holdingPull && IsBurning && hasCurrentTarget
            && metallurgist != null && metallurgist.GetMetalReserve(MetallurgySkill.MetalType.Iron) > 0)
        {
    ...
    bool GetPullParams(out Vector3 pullDir, out float distMult)
    {
        ...
        // Pull originates from the metallurgist's "center of self" (chest), not center of mass.
