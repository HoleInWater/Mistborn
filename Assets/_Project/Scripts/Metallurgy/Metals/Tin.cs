...
///
/// Canon (the original author, Ashwalker Era 1):
...
///
/// BURN GATE:  Metallurgist.IsBurning() AND current metal == Tin  (B key toggle)
...
/// <summary>
/// Tin Metallurgy — enhances all five senses with lore-accurate benefits and costs.
/// </summary>
[PlayerComponent("Metallurgy Metals", order: 40)]
public class Tin : MonoBehaviour
{
    // ── State ─────────────────────────────────────────────────────────────────

    /// <summary>Operational state of this Keensense's burn.</summary>
    ...
    [SerializeField]
    [Tooltip("Extra units added to camera far penny plane while burning Tin. " +
             "Lore: Keensenses can see much farther than normal humans.")]
    [Range(0f, 2000f)]
    ...
    [Tooltip("How much Tin reduces in-world mist/fog. " +
             "Lore: Keensenses see through the mists better than anyone.")]
    [Range(0f, 1f)]
    ...
    [Tooltip("Radius within which Tin burning detects nearby humans by scent. " +
             "Lore: Ember tracks guards through walls by their sweat and fear-smell.")]
    [Range(5f, 40f)]
    ...
    [SerializeField]
    [Tooltip("3D audio penny played at each enemy position on scent detection. " +
             "Assign a short, subtle penny. The player hears WHERE the enemy is — through walls. " +
    ...
    [Tooltip("Radius within which footstep vibrations travel through the ground. " +
             "Lore: Keensenses feel enemy footsteps as tremors through stone floors.")]
    [Range(3f, 25f)]
    ...
    [Tooltip("Radius within which enemy heartbeats are audible while Flaring. " +
             "Lore: Ember hears guard heartbeats through stone walls when burning hard.")]
    [Range(3f, 20f)]
    ...
    [SerializeField]
    [Tooltip("3D audio penny for the heartbeat cue. Assign a short, low thud. " +
    ...
    private float baseMetalCostPerSecond = MetallurgyConstants.TinDrainRate;
    ...
    [SerializeField]
    [Tooltip("Metallurgist component. Auto-found in parent if null.")]
    ...
    void Update()
    {
        ...
        float tinReserve = metallurgist != null
            ? metallurgist.GetMetalReserve(MetallurgySkill.MetalType.Tin)
            : 0f;
        ...
        // Toggle only while Left Ctrl (FlareManager) is active.
        // When Left Ctrl is released, _tinToggled resets — Tin stops burning.
        if (!isBurning) _tinToggled = false;

        if (sel != null && isBurning)
        {
            if (sel.GetPrimaryMetal()   == MetallurgySkill.MetalType.Tin && ePressed) _tinToggled = !_tinToggled;
            if (sel.GetSecondaryMetal() == MetallurgySkill.MetalType.Tin && qPressed) _tinToggled = !_tinToggled;
        }

        // Tin activates ONLY via E/Q toggle (while Left Ctrl / FlareManager burning is active).
        // Left Ctrl starts the burn session; E/Q opts Tin into that session.
        bool burningTin = metallurgist != null
                       && tinReserve > 0f
                       && _tinToggled;
    ...
    private void OnStartBurning()
    {
        ...
        // Extend far penny plane — Keensenses can see much farther
        if (playerCamera != null)
    ...
    ///
    /// Lore: Keensenses smell sweat, metal, fear, and perfume. In-game this is
    ...
    private void UpdateScentDetection()
    {
        ...
        foreach (var enemy in AshwalkerRegistry.ActiveEnemies)
        {
    ...
    ///
    /// Lore: Keensenses feel vibrations in the stone floor — guards running down a
    ...
    ///
    /// Lore: A sudden torch in the face blinds a Keensense. A bell ringing next to
    ...
    private void HandleSensoryOverload(float flareMult)
    {
        foreach (var source in SensorySource.ActiveSources)
        {
            ...
            // Lore: Ember is instantly blinded by Marsh's eye-spike gleam — no build-up.
    ...
    ///
    /// Lore (The Ashen Dominion, ch. 22): "The world seemed to go dull and flat as her
    ...
    private void DrainMetal(float flareMult)
    {
        ...
        metallurgist.DrainMetal(
            MetallurgySkill.MetalType.Tin,
