...
/// <summary>
/// Implements the Brass Metallurgy ability (Soothe emotions).
/// Standardized to follow the Metallurgist-centric burn system.
/// </summary>
[PlayerComponent("Metallurgy Metals", order: 60)]
public class Brass : MonoBehaviour
{
    ...
    void Update()
    {
        isBurning = metallurgist != null && metallurgist.IsBurning() && metallurgist.GetCurrentMetal() == MetallurgySkill.MetalType.Brass;
    ...
    void SootheEmotions(float flareMult)
    {
        ...
        {
            // Lore: aluminum-lined helmets (Metalhunters) block emotional Metallurgy
            MetalhunterAI metalhunter = enemy.GetComponentInParent<MetalhunterAI>();
            if (metalhunter != null && metalhunter.IsImmuneToEmotionalMetallurgy()) continue;
