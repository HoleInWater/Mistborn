...
/// <summary>
/// Metallurgic skill tree with 3 tiers per metal: Efficiency, Power, Mastery.
/// Plus passive branches for Combat, Movement, and Survival.
/// Integrates with PlayerExperience for skill point spending.
/// </summary>
public class MetallurgicSkillTree : MonoBehaviour
{
    public static MetallurgicSkillTree Instance { get; private set; }
    ...
    public class SkillNode
    {
        ...
        public MetallurgySkill.MetalType metal;
        public int cost = 1;
    ...
    void PopulateSkillTree()
    {
        ...
        string[] metalNames = { "Steel", "Iron", "Pewter", "Tin", "Zinc", "Brass", "Copper", "Bronze",
                                "Gold", "Electrum", "Oraculum", "Maloraculum", "Aluminum", "Duralumin",
                                "Bendalloy", "Cadmium", "Chromium", "Nicrosil" };

        for (int i = 0; i < 16; i++)
        {
            MetallurgySkill.MetalType metal = (MetallurgySkill.MetalType)i;
        ...
        AddSkill("Combat_Damage1", "Striking Force I", "Increase melee damage by 10%",
            SkillCategory.CombatPassive, SkillTier.Tier1, MetallurgySkill.MetalType.Pewter, 1, 0.10f);
        AddSkill("Combat_Damage2", "Striking Force II", "Increase melee damage by 20%",
            SkillCategory.CombatPassive, SkillTier.Tier2, MetallurgySkill.MetalType.Pewter, 2, 0.20f,
            new[] { "Combat_Damage1" });
        AddSkill("Combat_CritChance", "Critical Edge", "10% critical hit chance",
            SkillCategory.CombatPassive, SkillTier.Tier2, MetallurgySkill.MetalType.Pewter, 2, 0.10f,
            new[] { "Combat_Damage1" });
        AddSkill("Combat_ParryWindow", "Perfect Parry", "Increase parry window by 50%",
            SkillCategory.CombatPassive, SkillTier.Tier3, MetallurgySkill.MetalType.Oraculum, 3, 0.50f,
            new[] { "Combat_CritChance" });
        AddSkill("Combat_ComboExtend", "Relentless Combos", "Extend combo timeout by 1 second",
            SkillCategory.CombatPassive, SkillTier.Tier3, MetallurgySkill.MetalType.Steel, 3, 1f,
            new[] { "Combat_Damage2" });

        // Movement
        AddSkill("Move_Speed1", "Fleet Feet I", "Increase movement speed by 10%",
            SkillCategory.MovementPassive, SkillTier.Tier1, MetallurgySkill.MetalType.Steel, 1, 0.10f);
        AddSkill("Move_Speed2", "Fleet Feet II", "Increase movement speed by 20%",
            SkillCategory.MovementPassive, SkillTier.Tier2, MetallurgySkill.MetalType.Steel, 2, 0.20f,
            new[] { "Move_Speed1" });
        AddSkill("Move_FallDamage", "Pewter Landing", "Reduce fall damage by 50%",
            SkillCategory.MovementPassive, SkillTier.Tier1, MetallurgySkill.MetalType.Pewter, 1, 0.50f);
        AddSkill("Move_WallRunDuration", "Wall Runner", "Increase wall run duration by 50%",
            SkillCategory.MovementPassive, SkillTier.Tier2, MetallurgySkill.MetalType.Steel, 2, 0.50f,
            new[] { "Move_Speed1" });
        AddSkill("Move_DoubleJump", "Metallurgic Leap", "Steel Push off ground for double jump",
            SkillCategory.MovementPassive, SkillTier.Tier3, MetallurgySkill.MetalType.Steel, 3, 1f,
            new[] { "Move_WallRunDuration" });

        // Survival
        AddSkill("Surv_HealthRegen", "Vital Reserves I", "Increase health regen by 25%",
            SkillCategory.SurvivalPassive, SkillTier.Tier1, MetallurgySkill.MetalType.Gold, 1, 0.25f);
        AddSkill("Surv_HealthRegen2", "Vital Reserves II", "Increase health regen by 50%",
            SkillCategory.SurvivalPassive, SkillTier.Tier2, MetallurgySkill.MetalType.Gold, 2, 0.50f,
            new[] { "Surv_HealthRegen" });
        AddSkill("Surv_MetalRegen", "Metal Conservation", "Passive metal reserve recovery +25%",
            SkillCategory.SurvivalPassive, SkillTier.Tier1, MetallurgySkill.MetalType.Copper, 1, 0.25f);
        AddSkill("Surv_DamageReduction", "Iron Skin", "Reduce all damage taken by 10%",
            SkillCategory.SurvivalPassive, SkillTier.Tier2, MetallurgySkill.MetalType.Pewter, 2, 0.10f,
            new[] { "Surv_HealthRegen" });
        AddSkill("Surv_Immunity", "Metallurgic Resilience", "Immune to poison and sensory overload",
            SkillCategory.SurvivalPassive, SkillTier.Tier3, MetallurgySkill.MetalType.Tin, 3, 1f,
    ...
    void AddSkill(string id, string name, string desc, SkillCategory cat, SkillTier tier,
        MetallurgySkill.MetalType metal, int cost, float value, string[] prereqs = null)
    {
    ...
    void ApplySkillEffect(SkillNode skill)
    {
        ...
        // from the relevant gameplay systems (Metallurgist, PlayerHealth, etc.)
    }
    ...
    /// </summary>
    public float GetMetalEfficiencyBonus(MetallurgySkill.MetalType metal)
    {
        float bonus = 0f;
        foreach (var skill in allSkills)
        {
    ...
    /// </summary>
    public float GetMetalPowerBonus(MetallurgySkill.MetalType metal)
    {
        float bonus = 0f;
        foreach (var skill in allSkills)
        {
    ...
    public bool HasMastery(MetallurgySkill.MetalType metal)
    {
        foreach (var skill in allSkills)
        {
