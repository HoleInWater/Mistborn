...
/// </summary>
public static class MetallurgyPhysicsFormulas
{
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    /// <summary>
    /// Lore-accurate Metallurgic force (inverse-square, mass-product).
    /// F(a) = A × m₁ × m₂ / r²
    /// Handbook Section 2: A_vin ≈ 35,316 (without flaring), A_conservative ≈ 1,500
    /// </summary>
    public static float CalculateMetallurgicForce(float A, float metallurgistMass, float metalMass, float distance)
    {
        float r = Mathf.Max(0.5f, distance); // Prevent division by zero
        return A * metallurgistMass * metalMass / (r * r);
    }

    /// <summary>
    /// Metallurgic strength constant A, scaled by flare intensity.
    /// Base A = 1500 (conservative), flaring can multiply up to 2.5x.
    /// </summary>
    public static float GetMetallurgicStrength(float baseA, float flareMultiplier)
    {
    ...
    /// <summary>
    /// Coin acceleration from Metallurgic push.
    /// a = F(a) / m₂
    /// </summary>
    ...
    /// <summary>
    /// Power-limited Metallurgic force (community theory, Handbook Section 3).
    ...
    /// to deflect — the metallurgist's force is inversely proportional to the
    /// object's velocity.
    /// </summary>
    ...
    /// <summary>
    /// Constant-rate Storecrafted storage.
    /// S(t) = r₀ × t
    /// </summary>
    ...
    /// <summary>
    /// Storecraft weight factor (Handbook Section 7).
    /// W = m × g × f, where f is the Storecraft weight factor
    /// f < 1 when storing (lighter), f > 1 when tapping (heavier)
    /// </summary>
    public static float CalculateStorecraftedWeight(float mass, float gravityFactor)
    {
    ...
    /// <summary>
    /// Predict position under a sustained Metallurgic push/pull + gravity.
    ...
    /// <summary>Ember's calculated Metallurgic strength constant (unflared)</summary>
    ...
    // ── Official coin specs (Shire Post Mint, the original author-licensed) ────────────
    /// <summary>Clip mass: 3 g copper coin, 2 cm diameter (standard Launcher ammo)</summary>
    public const float CLIP_MASS = 0.003f;
    /// <summary>Crown mass: 15.5 g brass coin, 3 cm diameter</summary>
    ...
    /// <summary>Crown cross-section area: π × (0.015)² = 0.000707 m² (radius 1.5 cm)</summary>
    ...
    // ── Metal Densities (kg/m³, real-world values for metallurgic metals) ─────
    ...
    // ── MAG Travel Velocities (Ashwalker Adventure Game, canon-compliant) ────
    ...
    /// <summary>Standard Launcher travel speed: ~40 mph = 17.9 m/s = 23.5 units/s</summary>
