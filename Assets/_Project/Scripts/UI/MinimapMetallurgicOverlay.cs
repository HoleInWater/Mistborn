...
/// <summary>
/// Metallurgic overlay for the minimap system. Shows metal sources when burning
/// Bronze (Seeker), enemy positions when burning Tin, and quest markers.
/// Includes fog of war that clears as the player explores.
/// </summary>
public class MinimapMetallurgicOverlay : MonoBehaviour
{
    [Header("References")]
    ...
    void Start()
    {
        if (player == null)
        {
            ...
            if (playerObj != null)
            {
                ...
                metallurgist = playerObj.GetComponent<Metallurgist>();
            }
        }
    ...
    void Update()
    {
        if (player == null || metallurgist == null) return;
        ...
        if (metallurgist.IsMetalBurning(MetallurgySkill.MetalType.Tin))
        {
    ...
    void ScanForMetalSources()
    {
        ...
        foreach (var col in metals)
        {
            ...
            if (!metalMarkers.ContainsKey(id) && metalMarkerPrefab != null)
            {
                ...
                MetallurgicTarget target = col.GetComponent<MetallurgicTarget>();
    ...
    Color GetMetalMarkerColor(MetallurgySkill.MetalType metal)
    {
        switch (metal)
        {
    ...
    Color GetEnemyMarkerColor(EnemyAI.EnemyType type)
    {
        switch (type)
        {
            ...
            case EnemyAI.EnemyType.Bloodbrute:          return new Color(0.5f, 0, 0);
            case EnemyAI.EnemyType.IronSentinel: return new Color(0.8f, 0, 0.8f);
