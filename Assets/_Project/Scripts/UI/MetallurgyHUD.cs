...
/// <summary>
/// Metallurgy HUD — shows metal reserve bars, active metal indicator, flare state.
...
public class MetallurgyHUD : MonoBehaviour
{
    ...
    public Color oraculumColor    = new Color(0.9f,  0.9f,  1.0f);  // pale silver-blue (matches wheel)
    public Color revelumColor = new Color(0.50f, 0.28f, 0.62f); // deep violet
    ...
    void CreateBars()
    {
        if (barContainer == null || metallurgist == null) return;
    ...
    void Update()
    {
        ...
        if (updateTimer > 0f || metallurgist == null) return;
        updateTimer = UPDATE_INTERVAL;

        for (int i = 0; i < 16 && i < metalBars.Length && i < metallurgist.metalReserves.Length; i++)
        {
            if (metalBars[i] == null) continue;
            float reserve = metallurgist.metalReserves[i];
    ...
    Color GetMetalColor(int index)
    {
        switch (index)
        {
            ...
            case 8:  return oraculumColor;
            case 9:  return revelumColor;
