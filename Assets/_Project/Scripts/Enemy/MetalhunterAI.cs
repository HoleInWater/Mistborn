...
/// <summary>
/// Tactical squad-based AI for Metalhunters — elite anti-Metallurgist soldiers.
/// Lore: Metalhunters wear aluminum-lined helmets (immune to emotional Metallurgy),
/// carry non-metallic weapons (immune to Steel/Iron), and fight in coordinated squads.
/// </summary>
public class MetalhunterAI : MonoBehaviour, IDamageable
{
    [Header("Detection")]
    ...
    [Header("Anti-Metallurgy")]
    [Tooltip("Aluminum-lined helmet blocks Zinc/Brass emotional Metallurgy")]
    ...
    private MetalhunterAI squadLeader;
    private List<MetalhunterAI> squadMembers = new List<MetalhunterAI>();
    private bool isLeader = false;

    private float lastAttackTime;
    private float tacticalReassessTimer;
    private Vector3 formationTarget;
    private bool playerIsMetallurgist = false;
    private bool playerIsLauncher = false;
    private bool playerIsHauler = false;

    // ── Static Squad Registry ────────────────────────────────────────────
    private static List<MetalhunterAI> allMetalhunters = new List<MetalhunterAI>();

    [RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.SubsystemRegistration)]
    static void ResetStaticState() => allMetalhunters = new List<MetalhunterAI>();
    ...
    void Start()
    {
        ...
        if (playerObj != null)
        {
            ...
            Metallurgist allo = playerObj.GetComponent<Metallurgist>();
            if (allo != null)
            {
                playerIsMetallurgist = true;
                playerIsLauncher = allo.unlockedMetals[(int)MetallurgySkill.MetalType.Steel];
                playerIsHauler = allo.unlockedMetals[(int)MetallurgySkill.MetalType.Iron];
            }
        }

        allMetalhunters.Add(this);
        AssignSquad();
    }

    void OnDestroy()
    {
        allMetalhunters.Remove(this);
        if (isLeader)
        {
    ...
    void AssignSquad()
    {
        // Find nearby metalhunters and form squad
        squadMembers.Clear();
        MetalhunterAI closestLeader = null;
        float closestDist = float.MaxValue;

        foreach (var hk in allMetalhunters)
        {
    ...
    void AssessFormation(float distToPlayer)
    {
        ...
        if (playerIsLauncher && distToPlayer < 20f)
        {
            // Spread out vs Launcher — coins push in lines, spreading reduces multi-kills
            currentFormation = Formation.Flank;
        }
        else if (playerIsHauler && distToPlayer < 15f)
        {
            // Rush vs Hauler — close distance fast so pull is less effective
    ...
    /// <summary>
    /// Aluminum helmet blocks emotional Metallurgy (Zinc Riot / Brass Soothe).
    /// Called by Zinc.cs and Brass.cs before applying effects.
    /// </summary>
    public bool IsImmuneToEmotionalMetallurgy()
    {
    ...
    /// Called by MetallurgicTarget detection.
    /// </summary>
