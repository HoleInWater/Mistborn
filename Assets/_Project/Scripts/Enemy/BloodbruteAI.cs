...
/// <summary>
/// Lore-accurate Bloodbrute AI. Bloodbrute are massive blue-skinned creatures held together
/// by Bloodforged spikes. They grow larger with age (skin tears and stretches).
/// Rage mode escalates damage/speed. Ground slam AOE. Removing spikes makes them docile.
/// </summary>
public class BloodbruteAI : MonoBehaviour
{
    [Header("Size & Age")]
    ...
    [Header("Bloodforged Spikes")]
    public int spikeCount = 4;
    public bool spikesIntact = true;
    [Tooltip("Removing spikes makes Bloodbrute docile and eventually die")]
    public float spikeRemovalRange = 2f;

    [Header("Bloodbrute Sword")]
    public GameObject bloodbruteSwordPrefab;
    ...
    void Start()
    {
        ...
        // Size scales with age — older Bloodbrute are bigger
    ...
    void MeleeAttack(float distToPlayer)
    {
        ...
        if (Time.time - lastAttackTime >= attackCooldown)
        {
            ...
            // Bloodbrute attacks have knockback
    ...
    /// <summary>
    /// Remove a spike from the Bloodbrute. When all spikes are removed, it becomes docile and dies.
    /// Called by player interaction when close enough and Bloodbrute is stunned.
    /// </summary>
    public void RemoveSpike()
    {
        ...
        // Each spike removal weakens the Bloodbrute
        currentDamage *= 0.7f;
    ...
    void OnDeath()
    {
        ...
        if (dropSwordOnDeath && bloodbruteSwordPrefab != null)
        {
            Vector3 dropPos = transform.position + transform.forward * 1.5f;
            Instantiate(bloodbruteSwordPrefab, dropPos, Quaternion.identity);
        }
