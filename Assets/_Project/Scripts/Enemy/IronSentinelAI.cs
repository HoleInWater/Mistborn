...
/// <summary>
/// Multi-phase Iron Sentinel boss fight.
/// Phase 1 (100-60% HP): Steel/Iron pushes and pulls, melee attacks.
/// Phase 2 (60-30% HP): Burns Oraculum — dodges player attacks with future sight.
...
/// </summary>
public class IronSentinelAI : MonoBehaviour
{
    public enum BossPhase { Physical, Oraculum, AllBurn, Dead }
    ...
    public float oraculumDodgeWindow = 0.8f;
    ...
    void Update()
    {
        ...
        switch (currentPhase)
        {
            ...
            case BossPhase.Oraculum: UpdateOraculum(dist); break;
    ...
    void UpdateOraculum(float dist)
    {
        // Oraculum grants future sight — dodge incoming attacks
        if (!isDodging && ShouldDodge())
        {
            StartCoroutine(OraculumDodge());
        }
    ...
    bool ShouldDodge()
    {
        if (Time.time - lastDodgeTime < oraculumDodgeWindow * 2f) return false;
    ...
    IEnumerator OraculumDodge()
    {
        ...
        yield return new WaitForSecondsRealtime(oraculumDodgeWindow);
        isDodging = false;
    }
    ...
    /// <summary>
    /// Stun the Sentinel (e.g., from a powerful hit or Duralumin burst).
    ...
    void Die()
    {
        ...
        EventManager.TriggerEvent("SentinelDefeated");
