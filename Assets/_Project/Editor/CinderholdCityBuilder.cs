/* CinderholdCityBuilder.cs
 *
 * Editor tool: Ashwalker → World → Build Cinderhold City in Current Scene
 *
 * Generates a playable Cinderhold city using the same visual style as the
 * title sequence — primitives with TitleSequenceMaterialOverride for colors.
 * No prefabs needed. Creates:
 *   - Thornspire at the center (spires, walls, gate)
 *   - Noble district (tall buildings, keeps, wide streets)
 *   - Lowborn quarter (dense short hovels, narrow alleys)
 *   - Market squares, canals with bridges, city wall
 *   - Street-level details (lanterns, barrels, signs, etc.)
 *   - Metal objects tagged for Metallurgic interaction
...
public class CinderholdCityBuilder
{
    ...
    [MenuItem("Ashwalker/World/Build Cinderhold City in Current Scene")]
    public static void Build()
    {
        if (!EditorUtility.DisplayDialog("Build Cinderhold",
            "This will create a procedural Cinderhold city in the current scene.\n\n" +
        ...
        var root = new GameObject("Cinderhold_City");
        ...
        for (int gx = 0; gx < gridSize; gx++)
        {
            for (int gz = 0; gz < gridSize; gz++)
            {
                ...
                // Lowborn quarter (outer ring)
                else
                    BuildLowbornBlock(t, blockCenter, blockSize);
        ...
        Debug.Log($"[CinderholdCityBuilder] Built {root.transform.childCount} root objects, " +
    ...
    static void BuildThornspire(Transform parent, Vector3 center)
    {
        var ks = new GameObject("Thornspire");
    ...
    static void BuildLowbornBlock(Transform parent, Vector3 center, float size)
    {
    ...
    static void BuildCanal(Transform parent, Vector3 center, bool northSouth, float length)
    {
        ...
        // Metal railing — tagged as Metal for Metallurgy
    ...
    static void EnsureLayer(string name, int index)
    {
        ...
        if (string.IsNullOrEmpty(layers.GetArrayElementAtIndex(index).stringValue))
        {
            ...
            Debug.Log($"[CinderholdCityBuilder] Created layer '{name}' at index {index}");
        }
    }
}
#endif
